class quizMachine:
	...
	def pExecute(self, sql):
		...
		return data
	def getPwd(self, usr):
		sql = "SELECT username, password, DATE_FORMAT(time, '%Y-%m-%d') as time from users WHERE username like '%{0}%'".format(usr)
		print(sql)
		return self.pExecute(sql)
	...
	def favrank(self):
		...
		# result = self.toDict()
		return result
	def wLogin(self, openid):
		isBind = self.pExecute('SELECT username, token, time FROM users WHERE openid="{}"'.format(openid))
		if isBind:
			return isBind[0]
		return False
	def wBind(self, usr, pwd, openid):
		result = {
				'status': 201,
				'msg': '登录成功',
				'usr': usr
		...
		# 是否已绑定
		print(openid)
		userData = self.pExecute('SELECT username, token, DATE_FORMAT(time,"%Y-%m-%d") as time FROM users WHERE openid="{}"'.format(openid))
		if userData:
			userData = userData[0]
			...
			return result
		# 账号密码错误
		sql = 'SELECT openid, username, token, `time`  FROM users WHERE username="{}" and password="{}"'.format(usr, pwd)
		userData = self.pExecute(sql)
		print('user: ', userData, sql)
		if userData:
			
			userData = userData[0]
			uopenid = userData['openid']
			userData['usr'] = userData['username']
			del userData['username']
			result['usr'] = userData
			print('获取到的openid：', uopenid)
			# 未绑定openid
			if not userData['openid']:
				insert = self.pExecute('UPDATE users SET openid="{}" WHERE username="{}"'.format(openid, usr))
				if insert:
					# 绑定成功
					result['status'] = 200
					result['msg'] = '绑定成功'
