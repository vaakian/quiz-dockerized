...
def _msg():
	...
	html = '<h4>微信小程序上线，使用微信扫码体验。<br>或微信搜索：技能高考计算机</h4><img src="http://ww1.sinaimg.cn/large/7de5ef2egy1g1xuxeaz9lj209k09kjt0.jpg" width="100%" />'
	data = {
		'status': 301,
		'html': html
		}
...
def _bind():
	...
	rsp = httpGet(url)
	result = eval(rsp.text)
	# 通过code获取的openid 查看是否绑定
	result = quiz.wBind(usr, pwd, result['openid'])
	return jsonify(result)
